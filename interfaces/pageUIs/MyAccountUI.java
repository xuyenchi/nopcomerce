package pageUIs;

public class MyAccountUI {
    public static final String ACCOUNT_LINK = "//div[@class='account-cart-wrapper']//span[text()='Account']";
    public static final String MYACCOUNT_LINK = "//div[@id='header-account']//a[text()='My Account']";
    public static final String REGISTER_SUCCESS_MESSAGE = "//li[@class='success-msg']//span";
    public static final String CONTACT_INFO_LABEL = "//h3[text()='Contact Information']/parent::div/following-sibling::div/p";
}
